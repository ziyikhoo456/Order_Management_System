...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        if ($newquantity == 0){
            $delete = "DELETE FROM cart WHERE custID='".$_SESSION['ID']."' AND prodID ='".$row['prodID']."' ;";
            mysqli_query($conn, $delete) or die(mysqli_error($conn));
        }
        else{
        $update="UPDATE cart set grandTotal='".$newquantity."' WHERE custID='".$_SESSION['ID']."' AND prodID ='".$row['prodID']."' ;";
